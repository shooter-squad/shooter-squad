...
class DRQNAgent(object):
    ...
    def choose_action(self, observation): # intially very exploratory, later becomes greedy
        if np.random.random() > self.epsilon:
            state = T.tensor([observation],dtype=T.float).to(self.q_eval.device) # NOTE: state: [1, 84]
    ...
    def learn(self):
        ...
        states, actions, rewards, states_, dones = self.sample_memory() # NOTE:  actions: [32]; dones: [32]; rewards:[32]
